...
export const Rank = (props: {
  ...
  /** ランクの現在値変更 */
  const onChange = (_event: React.MouseEvent<HTMLElement>, newRankValue: string | null) => {
  ...
  return (
    <>
      ...
      <Grid container css={formMarginStyle}>
        {/* ランクセレクトボックス */}
        <Grid item xs={4}>
          <TextField css={formMarginStyle} defaultValue='' fullWidth id='rank' select size='small' variant='outlined' {...props.register('rank')}>
        ...
        {/* ランク一致 */}
        <Grid item xs={7}>
          <ToggleButtonGroup css={formMarginStyle} fullWidth exclusive size='small' onChange={onChange} value={props.rankRange}>
            ...
            <ToggleButton aria-label='3' css={selectBoxStyle} value='3'>
              <span>以下</span>
            </ToggleButton>
          </ToggleButtonGroup>
        </Grid>
