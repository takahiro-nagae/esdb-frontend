...
describe('SearchListHead', () => {
  ...
  test('エンチャント名昇順', async () => {
    ...
    expect(screen.getByText('エンチャント名').children[0]).toHaveTextContent(
      'sorted ascending',
    );
    expect(screen.getByText('位置').children[0]).not.toHaveTextContent(
      'sorted ascending',
    );
  ...
  test('効果が表示された時の表示確認', async () => {
    ...
    // 値の昇順にソートされていることの確認
    expect(screen.getByText('値').children[0]).toHaveTextContent(
      'sorted ascending',
    );
    expect(
